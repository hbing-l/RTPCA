...
#########################################PoseTransformer
print("build model...")
model_pos_train =  RTPCA(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
        num_heads=8, mlp_ratio=2., qkv_bias=True, qk_scale=None,drop_path_rate=0.1)

model_pos =  RTPCA(num_frame=receptive_field, num_joints=num_joints, in_chans=2, embed_dim_ratio=args.cs, depth=args.dep,
...
def evaluate(test_generator, action=None, return_predictions=False, use_trajectory_model=False, newmodel=None):
    ...
    with torch.no_grad():
        ...
        for _, batch, batch_2d in test_generator.next_epoch():
            ...
            predicted_3d_pos_next = torch.cat((predicted_3d_pos[:1, :, :], predicted_3d_pos), dim=0)
            predicted_3d_pos_next = predicted_3d_pos_next[:-1, :, :]
            predicted_3d_pos_accel = predicted_3d_pos - predicted_3d_pos_next
            
            inputs_3d_next = torch.cat((inputs_3d[:1, :, :], inputs_3d), dim=0)
            inputs_3d_next = inputs_3d_next[:-1, :, :]
            inputs_3d_accel = inputs_3d - inputs_3d_next
